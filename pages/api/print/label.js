  ...
  try {
    ...
    if (!upstream) {
      ...
      return res.status(all404 ? 404 : 502).json({
        ok: false,
        error: all404
          ? "Print label endpoint not found on configured upstreams"
          : "Print label upstream request failed on all configured upstreams",
        attempts,
      });
    }
    ...
    if (!upstream.ok && !responsePayload.error) {
      responsePayload.error = `Print label upstream returned ${upstream.status}`;
    }
    ...
    return res.status(isAbort ? 504 : 502).json({
      ok: false,
      error: isAbort ? "Print label upstream timeout" : "Print label upstream request failed",
