...
const BLUEFOLDER_SERVICE_REQUESTS_LIST_PATH = "/serviceRequests/list.aspx";

function normalizeBlueFolderListPath(value) {
  const path = envValue(value || BLUEFOLDER_SERVICE_REQUESTS_LIST_PATH);
  return /\/workOrders\/list\.aspx(?:[?#].*)?$/i.test(path)
    ? BLUEFOLDER_SERVICE_REQUESTS_LIST_PATH
    : path;
}
...
export function getBlueFolderSlackSettings() {
  ...
  return {
    ...
    bluefolderApiToken,
    bluefolderWorkOrdersPath: normalizeBlueFolderListPath(
      process.env.BLUEFOLDER_WORK_ORDERS_PATH ||
        BLUEFOLDER_SERVICE_REQUESTS_LIST_PATH
    ),
    ...
    bluefolderListStatus: envValue(
      process.env.BLUEFOLDER_LIST_STATUS || "open"
    ),
...
function collectNamedObjects(node, key, output = []) {
  if (!node || typeof node !== "object") return output;
  if (Array.isArray(node)) {
    node.forEach((entry) => collectNamedObjects(entry, key, output));
    return output;
  }
  Object.entries(node).forEach(([childKey, value]) => {
    if (childKey === key) {
      for (const entry of asArray(value)) {
        if (entry && typeof entry === "object") output.push(entry);
      }
    }
    collectNamedObjects(value, key, output);
  });
  return output;
}

function findScalar(node, keys) {
  ...
  for (const value of Object.values(node)) {
    ...
    const nested = findScalar(value, keys);
...
export class BlueFolderClient {
  ...
  async listRecentWorkOrders() {
    ...
    if (path.toLowerCase().includes(".aspx")) {
      const statusFilter = this.settings.bluefolderListStatus
        ? `<status>${xmlEscape(this.settings.bluefolderListStatus)}</status>`
        : "";
      const result = await this.request("POST", path, {
        data: `<request><serviceRequestList><listType>full</listType>${statusFilter}</serviceRequestList></request>`,
        headers: {
      ...
      if (apiError) {
        throw new Error(`BlueFolder list returned an API error on ${path}: ${apiError}`);
      }
  ...
  async getWorkOrderByExternalId(externalId) {
    const safeExternalId = xmlEscape(externalId);
    const result = await this.request("POST", "/serviceRequests/get.aspx", {
      data:
        `<request><externalId>${safeExternalId}</externalId>` +
        "<groupItemsByType>false</groupItemsByType></request>",
      headers: {
        "Content-Type": "text/xml",
        Accept: "text/xml, application/xml, application/json",
      },
    });
    if (!payloadStatusOk(result.payload)) return null;
    const matches = collectNamedObjects(result.payload, "serviceRequest");
    return matches[0] || result.payload?.serviceRequest || null;
  }

  async listCustomers({ listType = "basic" } = {}) {
    const normalizedListType = String(listType || "basic").toLowerCase() === "full"
      ? "full"
      : "basic";
    const result = await this.request("POST", "/customers/list.aspx", {
      data:
        `<request><customerList><listType>${normalizedListType}</listType>` +
    ...
    const apiError = payloadErrorMessage(result.payload);
    if (apiError) throw new Error(`BlueFolder customer list failed: ${apiError}`);
    return collectNamedObjects(result.payload, "customer");
  }

  async getCustomer(customerId) {
    const normalizedCustomerId = String(customerId || "").trim();
    if (!normalizedCustomerId) throw new Error("BlueFolder customer ID is required.");
    const result = await this.request("POST", "/customers/get.aspx", {
      data:
        `<request><customerId>${xmlEscape(normalizedCustomerId)}</customerId>` +
        "</request>",
      headers: {
        "Content-Type": "text/xml",
        Accept: "text/xml, application/xml, application/json",
    ...
    const apiError = payloadErrorMessage(result.payload);
    if (apiError) throw new Error(`BlueFolder customer lookup failed: ${apiError}`);
    const matches = collectNamedObjects(result.payload, "customer");
    return matches[0] || result.payload?.customer || null;
  }

  async listUsers() {
    const result = await this.request("POST", "/users/list.aspx", {
      data: "<request><userList><listType>full</listType></userList></request>",
      headers: {
        "Content-Type": "text/xml",
        Accept: "text/xml, application/xml, application/json",
      },
    ...
    const apiError = payloadErrorMessage(result.payload);
    if (apiError) throw new Error(`BlueFolder user list failed: ${apiError}`);
    return collectNamedObjects(result.payload, "user");
  }
  ...
  async createServiceRequest({
    ...
    const result = await this.request("POST", "/serviceRequests/add.aspx", {
      data:
        "<request><serviceRequestAdd>" +
        (customerId ? `<customerId>${xmlEscape(customerId)}</customerId>` : "") +
        `<description>${xmlEscape(String(description || "").slice(0, 100))}</description>` +
        `<detailedDescription>${xmlEscape(detailedDescription)}</detailedDescription>` +
        `<externalId>${xmlEscape(externalId)}</externalId>` +
        `<notifyCustomer>false</notifyCustomer>` +
        (priority ? `<priority>${xmlEscape(String(priority).slice(0, 50))}</priority>` : "") +
        (purchaseOrderNo
          ? `<purchaseOrderNo>${xmlEscape(String(purchaseOrderNo).slice(0, 25))}</purchaseOrderNo>`
          : "") +
        "<sourceName>AIS Website</sourceName>" +
        (sourceId ? `<sourceId>${xmlEscape(String(sourceId).slice(0, 50))}</sourceId>` : "") +
        "<status>Open</status>" +
        "</serviceRequestAdd></request>",
      headers: {
        "Content-Type": "text/xml",
        Accept: "text/xml, application/xml, application/json",
      },
    ...
    const apiError = payloadErrorMessage(result.payload);
    if (apiError) throw new Error(`BlueFolder service request creation failed: ${apiError}`);
    const serviceRequestId = findScalar(result.payload, [
      "serviceRequestId",
      "serviceRequestID",
    ]);
    if (!serviceRequestId) {
      throw new Error("BlueFolder did not return a service request ID.");
    }
    return { serviceRequestId, payload: result.payload };
  }

  async addAssignment(serviceRequestId, userIds = [], assignmentComment = "") {
    const ids = Array.from(
      new Set(asArray(userIds).map((value) => String(value || "").trim()).filter(Boolean))
    );
    if (!ids.length) return null;
    const result = await this.request("POST", "/serviceRequests/addAssignment.aspx", {
      data:
        "<request><serviceRequestAssignmentAdd>" +
        `<serviceRequestId>${xmlEscape(serviceRequestId)}</serviceRequestId>` +
        `<assignedTo>${ids
          .map((userId) => `<userId>${xmlEscape(userId)}</userId>`)
          .join("")}</assignedTo>` +
        (assignmentComment
          ? `<assignmentComment>${xmlEscape(assignmentComment)}</assignmentComment>`
          : "") +
        "<notifyCustomer>false</notifyCustomer>" +
        "</serviceRequestAssignmentAdd></request>",
      headers: {
        "Content-Type": "text/xml",
        Accept: "text/xml, application/xml, application/json",
      },
    ...
    const apiError = payloadErrorMessage(result.payload);
    if (apiError) throw new Error(`BlueFolder assignment failed: ${apiError}`);
  ...
  async addAttachment({
    ...
    fileName,
    contentType,
    description = "AIS website service request attachment",
  }) {
    const result = await this.request(
      ...
      {
        data:
          "<request><attachmentAdd>" +
          `<serviceRequestId>${xmlEscape(serviceRequestId)}</serviceRequestId>` +
          "<isPublic>false</isPublic>" +
          `<attachmentContent>${Buffer.from(content).toString("base64")}</attachmentContent>` +
          `<attachmentFileName>${xmlEscape(fileName)}</attachmentFileName>` +
          `<attachmentDescription>${xmlEscape(description)}</attachmentDescription>` +
          `<attachmentContentType>${xmlEscape(contentType)}</attachmentContentType>` +
          "</attachmentAdd></request>",
        headers: {
          "Content-Type": "text/xml",
          Accept: "text/xml, application/xml, application/json",
        },
    ...
    const apiError = payloadErrorMessage(result.payload);
    if (apiError) throw new Error(`BlueFolder attachment upload failed: ${apiError}`);
    const attachmentToken = findScalar(result.payload, ["attachmentToken", "token"]);
    if (!attachmentToken) throw new Error("BlueFolder did not return an attachment token.");
    return { attachmentToken, payload: result.payload };
  }
  ...
  async updateSlackLink(workOrderId, slackUrl) {
    if (this.isXmlMode()) {
      try {
        await this.editCustomFieldWithSlackLink(workOrderId, slackUrl);
      } catch (error) {
        console.warn(
          `[BlueFolder] Custom-field write-back failed for ${workOrderId}; falling back to a private comment:`,
          error
        );
  ...
  async editCustomFieldWithSlackLink(workOrderId, slackUrl) {
    const safeId = xmlEscape(workOrderId);
    const label = xmlEscape(this.settings.bluefolderLinkFieldLabel);
    const value = xmlEscape(slackUrl);
    const payload = `<request><serviceRequestEdit><serviceRequestId>${safeId}</serviceRequestId><customFields><customField name="${label}">${value}</customField></customFields></serviceRequestEdit></request>`;
    const result = await this.request("POST", "/serviceRequests/edit.aspx", {
      data: payload,
      headers: {
        "Content-Type": "text/xml",
        Accept: "text/xml, application/xml, application/json",
    ...
    if (!payloadStatusOk(result.payload)) {
      throw new Error(
        `BlueFolder serviceRequests/edit.aspx failed: ${
          payloadErrorMessage(result.payload) || "unknown API response"
        }`
      );
    }
  }
