...
function callbackTokenMatches(token, expectedHash) {
  const candidate = cleanText(token, 160);
  const expected = cleanText(expectedHash, 64).toLowerCase();
  if (!TOKEN_PATTERN.test(candidate) || !/^[a-f0-9]{64}$/.test(expected)) {
    return false;
  }
...
function publicScanEvent(data = {}) {
  ...
  return {
    ...
    storageUnitId: resolution.storageUnitId || null,
    canonicalCode: resolution.canonicalCode || null,
...
function getBridgeConfiguration(action, env = process.env) {
  ...
  const explicitName = action === "start"
  ...
  if (isolatedEnvironment) {
    ...
    return {
      ...
      candidates: [action === "start" ? exactStart : exactStop],
      production,
    };
  }
...
async function callScannerBridge(action, payload, options = {}) {
  ...
  const attempts = [];
  for (const targetUrl of config.candidates) {
...
async function ingestStorageScanEvent({
  ...
  if (!initialSnapshot.exists) fail(404, "session_not_found", "The scan session could not be found.");
  const initialSession = initialSnapshot.data() || {};
  if (!callbackTokenMatches(callbackToken, initialSession.callbackTokenHash)) {
  ...
  const resolution = sanitizeResolution(
    await resolveStorageScanCode(db, eventInput.code, {
  ...
  const result = await db.runTransaction(async (transaction) => {
    ...
    if (!callbackTokenMatches(callbackToken, currentSession.callbackTokenHash)) {
      fail(401, "invalid_callback_token", "The scan callback credential is invalid.");
    }
