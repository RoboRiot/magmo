...
async function resolveItemScan({ db, rawCode, transaction }) {
  ...
  const documentReads = lookupValues.map((documentId) =>
    readReference(collection.doc(documentId), transaction)
  );
  const queryReads = SKU_FIELDS.map((field) => {
    ...
    return readReference(query, transaction);
  });
  const [documentSnapshots, querySnapshots] = await Promise.all([
    Promise.all(documentReads),
    Promise.all(queryReads),
  ]);
