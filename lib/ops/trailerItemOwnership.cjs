...
function resolvedBranchOwnership({
  ...
  // Client/site is an item-level historical snapshot in association schema v2.
  // Never delete it when adding explicit trailer ownership. A separate dated
  // migration resolves missing client snapshots from trailer location history.
  const deleteFields = [];
  const changed =
