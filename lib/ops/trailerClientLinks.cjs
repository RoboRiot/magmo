...
function connectionSelectionForClient({
  ...
  const currentTrailer = trailers.find(
    (trailer) => clean(trailer?.id, 180) === clean(currentTrailerId, 180)
  );
  const trailer =
    currentTrailer && trailerClientId(currentTrailer) === normalizedClientId
      ? currentTrailer
      : autoTrailerForClient(trailers, normalizedClientId);
  return {
    clientId: normalizedClientId,
    trailerId: clean(trailer?.id, 180),
    machineId: trailer
      ? trailerMachineId(trailer) || clean(currentMachineId, 180)
      : clean(currentMachineId, 180),
  };
}
